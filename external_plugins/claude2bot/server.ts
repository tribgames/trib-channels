...
const BASE_INSTRUCTIONS = [
  ...
  'Messages may include <memory-context> blocks generated by the system. Use them as supporting context and never repeat the tags themselves.',
...
mcp.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    {
      name: 'memory_cycle',
      description: 'Run memory management operations: sleep (full cycle), flush (consolidate pending), rebuild (recent), prune (cleanup), status.',
      inputSchema: {
        type: 'object' as const,
        properties: {
      ...
      case 'memory_cycle': {
        ...
        try {
          if (mcAction === 'status') {
            ...
            await sleepCycle(ws)
            result = { content: [{ type: 'text', text: 'Sleep cycle completed.' }] }

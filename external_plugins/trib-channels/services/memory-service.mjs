 ...
 * Port: 3350-3357 (written to $TMPDIR/trib-channels/memory-port)
 */
...
const DATA_DIR = process.env.CLAUDE_PLUGIN_DATA
  ...
  || (() => {
    ...
    const candidates = [
      path.join(os.homedir(), '.claude', 'plugins', 'data', 'trib-channels-trib-channels'),
      path.join(os.homedir(), '.claude', 'plugins', 'data', 'trib-channels'),
    ]
...
const mcp = new Server(
  { name: 'memory-service', version: '1.0.0' },
  { capabilities: { tools: {} } },
)
  ...
  try {
    ...
    if (req.url === '/episode') {
      const id = store.appendEpisode({
        ...
        backend: body.backend || 'trib-channels',

...
```jsonc
{
  "backend": "discord",           // "discord" | "telegram"
  "discord": { "token": "..." },
  "channelsConfig": {
...
UserPromptSubmit (user-prompt.cjs)
  - transcript offset 저장
  - 유저 메시지에 리액션 추가
  - sentCount 리셋 (새 메시지면 0)
  │
  ▼
Claude 응답 생성 중...
  │
  ├─▶ PreToolUse (pre-tool.cjs)  [도구 호출 전마다]
  │     - transcript에서 미전송 텍스트 추출
  │     - MD5 hash로 중복 전송 방지
  │     - Discord로 텍스트 전송
  │
  ├─▶ PostToolUse (post-tool.cjs)  [도구 호출 후마다]
  │     - 리액션 업데이트
  │     - pending text + 도구 로그 전송
  │     - MCP 도구는 'mcp'로 표시
  │
  ▼
Stop (stop.cjs)
  - 리액션 제거
  - 미전송 텍스트 최종 전송 (transcript 또는 fallback)
  - sessionIdle = true 설정
```

...
| 파일 | 역할 | 런타임 반영 |
|------|------|-------------|
| `config.json` | 백엔드, 채널, access, 스케줄, 음성 설정 | 채널/토큰/음성/스케줄 변경 후 세션 재시작 권장 |
...
| 정책 | 동작 |
|------|------|
| `pairing` | 페어링 코드 인증 후 DM 가능 |
| `allowlist` | allowFrom 유저만 DM 가능 |
| `disabled` | DM 비활성화 |
...
3. `config.json > access.allowFrom`에 유저 ID 포함 여부 확인
...
### 음성 인식 실패
1. `ffmpeg`, `whisper-cli`가 PATH에 있는지 확인
2. 모델 파일 경로 확인 (`voice.model`) 또는 기본 모델 설치 여부 확인
3. 필요 시 whisper-cli 수동 재설치

### 권한 버튼 무반응
1. `config.json > access.allowFrom`에 버튼 클릭 유저 포함 확인
2. 봇 권한: 메시지 수정 권한 필요
